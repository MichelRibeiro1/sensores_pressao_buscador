...
function filtrarSensores() {
    ...
    const table = document.getElementById("table");
    const dataFiltered = data.filter(item => item[1].trim() === med.trim()
        && item[2].trim() === saida.trim()
        && item[3].trim() === temp.trim()
        && item[4].trim() === exatidao.trim()
    );

    fillTable(table, dataFiltered);
}
